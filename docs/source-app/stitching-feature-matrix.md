...
| --- | --- | --- | --- | --- | --- | --- |
| `actChangeStitchingHoleShape` | `actChangeStitchingHoleShape`, `Open Pricking Iron Toolbox...` | Stitch tool menu or toolbox | `100691a9c`, `10069dbd8` | `partial` | high | LeatherCad has grouped/system/custom presets with pitch, width, height, tilt, blades, inversion, and source-shaped JSON import/export. Remaining gaps are native `.lccp` fixture validation and a fully dedicated source-style editor surface. |
...
| `actShowHideStitchingSimulator` | `Turn On/Off Stitching Simulator` | Stitch simulator toggle | `1006944a8` | `partial` | high | LeatherCad has simulator settings, modal controls, F6 toggle, quick thickness controls, and Leather Simulator coupling. Direction-arrow parity still needs validation. |
...
| `ShowStitchPattern` | `chkShowStitchPattern`, `Show Stitches` | Simulator visibility checkbox | `100699e64`, `100a28850`, `100a2a05c`, `1009cfe54` | `implemented` | high | Persisted simulator pattern visibility exists and Leather Simulator forces it on. |
| `ShowEven/ShowOddStitches` | `chkShowEvenStitches`, `chkShowOddStitches` | Simulator visibility controls | `100699fb0`, `10069a0ec` | `implemented` | high | Even/odd visibility is modeled, persisted, exposed in the simulator modal, and quick-cycled from the keyboard. |
| `Pricking Iron Toolbox` | `Pricking Iron Toolbox`, `btnDeletePrickingIronGroup`, `CannotDeleteSystePrickingIrons` | Dedicated preset editor dialog | `10069dbd8`, `100961934` | `partial` | high | Grouped presets, system guards, blade count, width/height, tilt, inversion, and import/export exist. Remaining gap is replacing prompt-driven creation with a richer dedicated editor and validating a native `.lccp` sample. |
...
- The clearest missing depth is around:
  - native `.lccp` and desktop-generated `.lcc` fixture validation,
  - source-style pricking iron editor UX,

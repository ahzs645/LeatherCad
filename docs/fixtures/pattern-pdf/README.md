...
published for personal use at <https://makesupply-leather.com>. It is here as
test data — a real published sheet exercises the import in ways a synthetic one
cannot: a maker's logo that must not be read as a piece, a stitch run that turns
two corners, a pocket whose free edge is a single long curve, and a keychain tab
whose only clue that it folds is that its two three-hole runs match.

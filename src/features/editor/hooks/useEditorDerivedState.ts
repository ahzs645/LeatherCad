...
import { useEditorDerivedAnnotations } from './useEditorDerivedAnnotations'
...
export function useEditorDerivedState(params: UseEditorDerivedStateParams) {
  ...
  const {
    ...
    pieceInterfaces: state.pieceInterfaces,
    assemblyConnections: state.assemblyConnections,
  ...
  const currentSnapshot = useMemo<EditorSnapshot>(
    () => ({
      ...
      pieceInterfaces: deepClone(pieceInterfaces),
      assemblyConnections: deepClone(assemblyConnections),

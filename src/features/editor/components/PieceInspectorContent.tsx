...
export type PieceInspectorContentProps = {
...
export function PieceInspectorContent({
  piece,
  grainline,
  pieceLabel,
  patternLabel,
  ...
  if (!piece) {
    return (
      <div className="control-block">
        <h3>Piece Inspector</h3>
        <p className="hint">Select or create a pattern piece to edit its metadata, seam allowances, labels, and 3D placement details.</p>
      </div>
    )
  }
      ...
      <div className="control-block">
        ...
        <label className="layer-field">
          ...
          <input
            ...
            value={piece.name}
            onChange={(event) => onUpdatePiece({ name: event.target.value })}
          />
        </label>
        <label className="layer-field">
          <span>Quantity</span>
          <input
            ...
            onChange={(event) =>
              onUpdatePiece({
                quantity: Math.max(1, Math.round(parseNumber(event.target.value, piece.quantity)) || 1),
              })
            }
          />
        </label>

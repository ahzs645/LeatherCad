...
const baseParams = {
  lineTypesById: {
    cut: { id: 'cut', name: 'Cut', role: 'cut' as const, visible: true, locked: false, style: 'solid' as const, color: '#111827' },
  },
  selectedShapeIdSet: new Set<string>(),
  sketchWorkspaceMode: 'sketch' as const,
...
describe('buildCanvasRenderModel', () => {
  ...
  it('includes non-shape entities in the combined render stream', () => {
    const model = buildCanvasRenderModel({
      ...
      foldLines: [{ id: 'fold', name: 'Fold', start: { x: 0, y: 1 }, end: { x: 30, y: 1 }, angleDeg: 90, maxAngleDeg: 180 }],
